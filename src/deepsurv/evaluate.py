...
def evaluate_model(
    ...
    y_train_surv = Surv.from_dataframe(status, target, y_train)
    y_test_surv = Surv.from_dataframe(status, target, y_test)
    ...
    if verbose:
        print(f"IPCW C-index - Train: {train_ci_ipcw:.4f} | Test: {test_ci_ipcw:.4f}")
